...
###### Особенности
```
- HSI  реально от ~2 MHz до ~50 MHz 
- Flash  работает до ~30 MHz
- FastIO (чтение/запись в порт - 1 такт)
- Программа в RAM - overclock до ~100 MHz  (0ws)
- Потребление очень маленькое даже MaxRun
- При установке в startup файле сразу HSI = ~2 MHz (3 команды ASM)
      потребление на старте (без периферии)  ~290 мкА
- МИНУС - медленная работа с RAM - 4 такта ( ? арбитраж с DMA )      
```
...
###### Особенности
```
- HSI  реально от ~2 MHz до ~80 MHz 
- FastIO (чтение/запись в порт - 1 такт)
- Программа в RAM - overclock до ~80 MHz  (0ws)
- Потребление очень маленькое даже MaxRun
- При установке в startup файле сразу HSI = ~2 MHz (3 команды ASM)
      потребление на старте (без периферии)  ~290 мкА
- Доступ в RAM - 2 такта ( нет DMA - сразу всё OK )      
```
...
Более подробно [ЗДЕСЬ](https://github.com/Xiamatsu/HC32L)
...
CH32V003 - 48MHz, Flash 16K, RAM 2K;  so8,so16  (2.7-5.5V)
  ...
  '+' ISA ( RV32EC ) простая и мощная 
  '+' есть ОУ

  '-' нет умножения в ISA 
  '-' в корпусах so8 и so16 нет SPI
  '-' LSI не может быть SYSCLK
  '-' ADC 10-bit
  '-' нет температурного датчика
  '-' потребление не маленькое в режимах Stop и Standby
  '-' потребление при старте высокое при HSI = 8 MHz
  '-' на минимальных частотах HCLK (94kHz - 750kHz) 
          потребление минимум 1.1 мА
